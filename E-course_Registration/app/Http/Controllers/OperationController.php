...
class OperationController extends Controller
{
    public function addDepartment(Request $request){
     ...
     public function addCourse(Request $request){
         ...
         $deptId = DB::table('courses')->insertGetId(
             ...
             ,'SEMESTER_NAME'=> $semesterName , 'DEPT_CODE'=>$departmentCode, 'CATEGORY'=>$category ,'TYPE'=>$type
             ]
         );
  ...
  public function editCourseInformation(Request $request){
      ...
      DB::table('courses')->where('COURSE_ID','=',$courseId)->update([
          'COURSE_CODE' => $courseCode, 'COURSE_NAME' => $courseName, 'COURSE_CREDIT'=> $courseCredit, 'COURSE_LEVEL'=> $courseLevel
          ,'SEMESTER_NAME'=> $semesterName , 'DEPT_CODE'=>$departmentCode,'TYPE'=>$type, 'CATEGORY'=>$category
      ]);
  ...
  public function addOfferedCourse($session_month,$year){
      Session::put('offeredExamYear', $year);
      Session::put('offeredSessoinMonth',$session_month);
      $studentSession=DB::table('batch_info')->select('ADMIT_SESSION')->orderBy('ADMIT_SESSION', 'asc')->get();
      Session::put('studentSession',$studentSession);
